...
/**
 * The code in book doesn't work correctly!
 * I have fixed it! 
 * 
 * @author PriteshJ
 *
...
public class TicTacToeClient extends Application implements TicTacToeConstants {
	...
	private boolean myTurn = false; // Controls if 'X' or 'O' gets drawn on GUI
	...
	public void start(Stage primaryStage) {
		...
		for(int i = 0; i<3; i++)
			for(int j = 0; j<3; j++) {
				cell[i][j] = new Cell(i, j); // Fixed the code in book
	...
	private void connectToServer() {
		...
		// Control the game on a separate thread (Within connectToServer())
		new Thread(() -> {
			try {
				...
				// Continue to play (Runs until anyone wins or is a draw)
	...
	public class Cell extends Pane {
		...
		public Cell(int row, int column) {
			this.row = row;
			this.column = column;
			this.setPrefSize(2000, 2000); // What happens without this? Shows a tiny dot
